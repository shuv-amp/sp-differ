...
fn build_output_records(
    ...
    let scan_pubkey = PublicKey::from_slice(&parsed.scan_pubkey).map_err(|_| Status::InvalidPubkey)?;
    let spend_pubkey =
...
mod taproot_negation_tests {
    ...
    fn run_case_payload(payload: &[u8]) -> Vec<u8> {
        ...
        let rc = sp_differ_worker_run(payload.as_ptr(), payload.len(), &mut output, &mut output_len);
    ...
    fn test_official_case_07_matches_expected_xonly_output() {
        ...
        let case = manifest["cases"]
            ...
            .unwrap();
        let expected_xonly = case["expected_outputs_xonly_groups"][0][0].as_str().unwrap();
